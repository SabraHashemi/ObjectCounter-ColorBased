		...
		color_space cs,	dist_type dt, unsigned colors_count, double colors_part)
{
	...
	if(dt == DT_KMEANS)
	{
		...
		{
		case CS_HSV:
			cv::cvtColor(img, img_cs, COLOR_BGR2HSV);
			break;
		case CS_BGR:
			img.copyTo(img_cs);
		}
...
double GetCIE76Dist(cv::Vec3i c1, cv::Vec3i c2, color_space cs)
{
	...
	{
	case CS_HSV:
		cv::cvtColor(colors, colors, COLOR_HSV2BGR);
		break;
	}
	cv::cvtColor(colors, colors, COLOR_BGR2Lab);
...
cv::Mat GetHist(cv::Mat img, color_space cs)
{
	...
	case CS_HSV:
	{
		cvtColor(img, img_colors, COLOR_BGR2HSV);
	}
		break;
	}
...
cv::Mat ShowColorsjust(std::vector<cv::Scalar> colors)
{
    Mat img_colors = Mat::zeros(Size(100,20),CV_8U);
    cv::Rect color_rect(cv::Point(0, 0), cv::Size(img_colors.cols / colors.size()-1, 20));
    for(unsigned i = 0; i < colors.size(); i++)
    {
        color_rect.x = (color_rect.width + 1) * i;
        cv::rectangle(img_colors, color_rect, colors[i], cv::FILLED);
